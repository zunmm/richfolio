...
Richfolio は単一のパイプラインに 19 以上の機能を詰め込んでいます — すべて無料枠の API で動作します。
...
## Watch List（リサーチ用ティッカー）

`config.json` のオプショナルな `watching` 配列は、目標配分にコミットせずに**スコアリングしてシグナルとして表示したい**ティッカーを追跡します。Watch ティッカーはポートフォリオティッカーと同じデータ取得 + AI パイプラインを通りますが、配分ベースのルールはバイパスします：

- 配分パーセンテージ計算から除外 — ポートフォリオ合計は 100% のまま
- 配分ギャップのルール（STRONG BUY に gap ≥ 2%、gap > 0% 必須）は**適用されません**
- STRONG BUY 最大 2 件上限のカウント対象外 — 該当する watch STRONG BUY はすべて表示
- `suggestedBuyValue` は常に 0 — ポジションサイズは手動で決定
- メールと Telegram で別の「Watch List」セクションに表示（黄色の見出し、破線の区切り）

参照できる配分ギャップがないため、watch STRONG BUY はより強いシグナル合流が必要です：≥1 個の価格レベルシグナル + ≥2 個のモメンタムシグナル + 重大なレッドフラグなし + バリュー評価 A/B（株式の場合）。後で目標ポートフォリオに加えるかもしれないリサーチ候補に watch list を活用してください — *「NVDA でポジションを始めるのに今は良いタイミング？」* — 配分計算を汚すことなく確認できます。

詳細スキーマは[設定 → Watch List](configuration#watch-list)を参照してください。
...
メールと Telegram に加えて、Richfolio は買いシグナルを公開ソーシャルアカウント — **X/Twitter**、**Facebook ページ**、**Threads** アカウント、**LinkedIn ページ** — に**オプションで**投稿できます（毎日モードとザラ場モードのみ。週次や refresh では決して投稿しません）。投稿はあえて汎用的にしてあります：**STRONG BUY** と **BUY** のシグナルのみで、それぞれティッカー、アクション、信頼度、短い理由を表示します。

プライベートな情報は一切開示されません — 配分、ギャップ、保有、推奨買い金額は**決して**投稿されず（サニタイザが AI の理由テキストからもそうした数値を除去します）、ポートフォリオと watch list のティッカーは「シグナル」として一様にまとめられるため、保有しているかどうかは決して明かされません。ティッカーは Facebook / Threads / LinkedIn ではクリック可能な `#hashtag` として描画され、リーチを高めるために設定可能な汎用ハッシュタグのセットも追加されます。すべての投稿は *「投資助言ではありません」* という免責事項で終わります。

各プラットフォームはそれぞれ独自の認証情報で独立してゲートされ、それぞれ独自のエラー境界の内側で投稿するため、ソーシャルの失敗がメール／Telegram のブリーフに影響することは決してありません。認証情報を追加するまでは完全にオフのままです。

セットアップ全体（Facebook、LinkedIn、X）と `config.json` のトグルについては[ソーシャル投稿](social-setup)を参照してください。
