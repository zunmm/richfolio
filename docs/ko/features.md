...
이메일과 Telegram 외에도, Richfolio는 일일 및 장중 모드에서(주간이나 refresh 모드에서는 절대 아님) 매수 신호를 공개 소셜 계정 — **X/Twitter**, **Facebook 페이지**, **Threads** 계정, **LinkedIn 페이지** — 에 **선택적으로** 게시할 수 있습니다. 게시물은 의도적으로 일반적입니다: **STRONG BUY**와 **BUY** 신호만 다루며, 각각 종목, 액션, 신뢰도, 그리고 짧은 이유를 보여줍니다.

비공개 정보는 전혀 공개되지 않습니다 — 배분, 격차, 보유 종목, 추천 매수 금액은 **절대** 게시되지 않으며(새니타이저가 AI의 이유 텍스트에서 그러한 수치도 제거합니다), 포트폴리오와 워치 리스트 티커는 "신호"로 동일하게 합쳐져 보유 여부가 드러나지 않습니다. 종목은 Facebook / Threads / LinkedIn에서 클릭 가능한 `#hashtag`로 렌더링되며, 도달률을 높이기 위해 설정 가능한 일반 해시태그 세트가 함께 덧붙습니다. 모든 게시물은 *"투자 자문이 아님(Not financial advice)"* 면책 문구로 끝납니다.

각 플랫폼은 자체 자격 증명에 따라 독립적으로 게이팅되며 자체 오류 경계 안에서 게시되므로, 소셜 게시 실패가 이메일/Telegram 브리핑에 영향을 주는 일은 없습니다. 자격 증명을 추가하기 전까지는 완전히 꺼져 있습니다.

전체 설정(Facebook, LinkedIn, X)과 `config.json` 토글은 [소셜 게시](social-setup)를 참고하세요.
