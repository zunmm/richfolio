...
除了電子郵件與 Telegram 之外,Richfolio 還能在每日與盤中模式下(絕不在每週或 refresh 模式)**選擇性地**將買進訊號發布到公開的社群頁面 — **X/Twitter**、**Facebook 粉絲專頁**、**Threads** 帳號與 **LinkedIn 頁面**。貼文刻意保持通用:只發布 **STRONG BUY** 與 **BUY** 訊號,各自顯示股票代碼、動作、信心度與簡短理由。

不會揭露任何隱私資訊 — 配置、缺口、持倉與建議買進金額**絕不會**被發布(同時有一個 sanitizer 會從 AI 的理由文字中剝除任何此類數字),且投資組合與觀察清單的標的會一律合併為「訊號」,因此持有狀況永遠不會被洩漏。在 Facebook / Threads / LinkedIn 上,股票代碼會呈現為可點擊的 `#hashtag`,並附加一組可設定的通用 hashtag 以提升觸及。每則貼文結尾都會附上*「非投資建議」*的免責聲明。

每個平台都各自以自己的憑證獨立啟用,並在各自的錯誤邊界內發文,因此社群發文失敗絕不會影響電子郵件/Telegram 簡報。在你加入憑證之前,它會完全保持關閉。

完整設定(Facebook、LinkedIn、X)以及 `config.json` 的開關詳見[社群發文](social-setup)。
