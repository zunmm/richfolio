...
除邮件和 Telegram 外,Richfolio 还可以**可选地**把买入信号发布到公开的社交页面 — **X/Twitter**、**Facebook 主页**和 **LinkedIn 主页** — 仅在每日和盘中模式下(绝不在每周或刷新模式)。发布内容刻意保持通用:只发 **STRONG BUY** 和 **BUY** 信号,每条显示股票代码、动作、置信度和一句简短理由。

不会泄露任何私密信息 — 配置、缺口、持仓和建议买入金额**绝不**发布,而且组合标的与观察列表标的会被统一合并为"信号",因此从不暴露持有情况。每条帖子都以*"非投资建议"*免责声明结尾。
